...
class DateTime
{
    ...
    public static function toGregorian($j_y, $j_m, $j_d)
    {
        ...
        $j_day_no = 365*$jy + self::divs($jy, 33)*8 + self::divs($jy%33+3, 4);
        for ($i=0; $i < $jm; ++$i)
            $j_day_no += $j_days_in_month[$i];
        ...
        $gy = 1600 + 400*self::divs($g_day_no, 146097);
        $g_day_no = $g_day_no % 146097;

        $leap = true;
        if ($g_day_no >= 36525) {
            $g_day_no--;
            $gy += 100*self::divs($g_day_no,  36524);
    ...
    private static function divs($a, $b)
    {
        return (int) ($a / $b);
    }
